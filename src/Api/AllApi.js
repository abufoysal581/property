...
const clientlogin = async (data) => {
    const LOGIN_ENDPOINT = `${process.env.REACT_APP_API_URL}/clientlogin`;
    try {
...
const clientregister = async (data) => {
    const SIGNUP_ENDPOINT = `${process.env.REACT_APP_API_URL}/clientregister`;
    try {
        let response = await axios({
            method: 'post',
            responsiveTYpe: 'json',
...
export { clientlogin, clientregister, logout }